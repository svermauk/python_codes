...
if __name__ == '__main__':
  ...
  windows = sys.argv[3:]
#  print prog, skip, glob_pattern, windows
...
#          if ln > skip and ln <= 28867880 and line.startswith('L9') and not 'dV/dlambda' in line:
          if line.startswith('Summary'):
               start=ln
          if line.startswith('End of dvdl summary'):
               end=ln
          if ln>start and start!=0 and ln<end:
               i+=1
          if i%10==0 and i!=0 and ln<end:

             dVdl.accumulate(float(line.split()[0]) )
